...
from utils.utils import (
    ...
    initialize_config,
    load_wav_simple_resample,
    downsample_16khz_to_8khz,
)
...
class preproc:
    ...
    def process(self, y):
        ...
        noisy_mag, noisy_phase = librosa.magphase(
            librosa.stft(
                y,
                n_fft=self.n_fft,
                hop_length=self.hop_length,
                window=get_window(self.n_fft),
            )
        )
        noisy_mag_tensor = torch.tensor(noisy_mag[None, ...], device=device).permute(
            0, 2, 1
        )  # (batch_size, T, F)
        assert noisy_mag_tensor.dim() == 3
        nn_features_tensor = 10.0 * torch.log10(
            noisy_mag_tensor**2 + self.logmag_epsilon
        )
...
class postproc:
    ...
    def process(self, noisy_mag, noisy_phase, gain_tensor):
        ...
        gain_mod_tensor = torch.maximum(
            gain_tensor * torch.sin(pi / 2 * gain_tensor),
            torch.tensor(self.min_gain, device=device),
        )
        ...
        pred_clean_y = librosa.istft(
            pred_clean_mag * noisy_phase,
            n_fft=self.n_fft,
            hop_length=self.hop_length,
            window=get_window(self.n_fft),
            center=False,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Speech enhancement using JUMPML NR")
    parser.add_argument(
        "-i",
        "--input_file",
        required=True,
        default="data/outdoor_mix.wav",
        type=str,
        help="Input wav file",
    )
    parser.add_argument(
        ...
        default="data/jumpmlnr_output.wav",
    )
    parser.add_argument(
        "-m",
        "--model_file",
        required=False,
        type=str,
        help="Pytorch JumpML model (.ptj) file",
        default="models/pretrained_models/jumpmlnr_midi.ptj",
    )
    parser.add_argument(
        "-g",
        "--minGain",
        required=False,
        type=float,
        help="Min. Gain in dB in range [-60, 0.0] dB",
        default=-30,
    )
    parser.add_argument(
        "-n",
    ...
    y, fs_input = load_wav_simple_resample(args.input_file, fs_target=16000)

    y_clean = run_pytorch_prediction(
        args.model_file, y, naturalness=nat, min_gain=min_gain
    )

    if fs_input == 8000:
        y_clean_8kHz = downsample_16khz_to_8khz(y_clean)
        wave.write(args.output_file, fs_input, y_clean_8kHz)
    else:
