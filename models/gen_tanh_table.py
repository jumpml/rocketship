...
def dump_table_Cfile(
    x, y, fname="include/tanh_table.h", datatype="int16_t", postfix=""
):
    f = open(fname, "w")
    table_len = len(y)
    printHeader(
        f,
        include_files=["<stdint.h>", "common_def.h"],
        fName=fname,
        autogen_str="by gen_tanh_table.py",
    )
    ...
    f.write(
        f"#define TANH_DELTAX{postfix} {convert_datatype(DELTA_X, datatype=datatype, nFracBits=15)}{typeStr}\n"
    )
    f.write(f"#define TANH_SCALEFAC{postfix} {SCALE_FAC}\n")
    f.write("\n\n")
    printVector(
        f, y, f"tanh_table{postfix}", datatype=datatype, nFracBits=15
    )  # ignores nFracBits when datatype is float
...
if __name__ == "__main__":
    ...
    dump_table_Cfile(
        TABLE_X,
        TANH_TABLE,
        fname="include/tanh_table_S16.h",
        datatype="int16_t",
        postfix="_S16",
    )
    validate_function()
