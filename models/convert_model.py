...
def printHeader(
    f,
    include_files=["signalsifter.h"],
    fName="signalsifter_weights.c",
    author="convert_model.py",
    autogen_str="from a Pytorch model and config file",
):
    """
...
def printModelStruct(f, model, structType="RNNModel", instanceName="ss_model"):
    ...
    for name, module_type, act_type, input_size, output_size in ModuleInfoGen(model):
        if module_type == "GRU":
            ...
            print(f"Unknown layer: {name} with shape: {input_size}x{output_size}")
...
def printVector(f, vector, name, datatype="float", nFracBits=7):
    ...
    f.write(
        f"DSP_RWDATA_IN_DRAM DSP_ALIGN16 static const {datatype} {name}[{len(v)}] = {{\n   "
    )
...
def create_model_header_file(
    fname="./include/signalsifter_config.h",
    w_datatype="int8_t",
    b_datatype="int8_t",
    w_fracBits=7,
    b_fracBits=7,
    ...
    f.write(
        f"\n#define FRAME_SIZE {hop_length}"
    )  # Application Frame Size. ToDo: Make this an Application config setting.
...
def dump_model_to_Cfile(
    model,
    src_fname="/tmp/nn_data.c",
    hdr_fname=None,
    w_datatype="int8_t",
    b_datatype="int16_t",
    H=128,
    ...
    create_model_header_file(
        fname=hdr_fname,
        w_datatype=w_datatype,
        b_datatype=b_datatype,
        w_fracBits=w_fracBits,
        b_fracBits=b_fracBits,
        H=H,
        io_size=io_size,
        fft_size=fft_size,
        hop_length=hop_length,
        logmag_epsilon=logmag_epsilon,
        input_intBits=input_intBits,
    )
    ...
    print(
        f"{hop_length} samples --> logMagSpec(FFT{fft_size}) --> NN Masker(IO={io_size}) --> ISTFT(FFT{fft_size}) --> {hop_length} samples"
    )
...
def dump_testvectors_Cfile(
    input,
    output,
    state_in,
    state_out,
    fname="test/testss/signalsifter_testvector.h",
    input_intBits=0,
):
    f = open(fname, "w")
    ...
    for idx, dt in enumerate(datatypes):
        ...
        printVector(
            f,
            input.data.numpy(),
            f"test_input{postfix[idx]}",
            datatype=dt,
            nFracBits=nFracBits - input_intBits,
        )
        printVector(
            f,
            output.data.numpy(),
            f"test_output{postfix[idx]}",
            datatype=dt,
            nFracBits=nFracBits,
        )
        printVector(
            f,
            state_in[0].data.numpy(),
            f"test_state_in1{postfix[idx]}",
            datatype=dt,
            nFracBits=nFracBits,
        )
        printVector(
            f,
            state_in[1].data.numpy(),
            f"test_state_in2{postfix[idx]}",
            datatype=dt,
            nFracBits=nFracBits,
        )
        printVector(
            f,
            state_in[2].data.numpy(),
            f"test_state_in3{postfix[idx]}",
            datatype=dt,
            nFracBits=nFracBits,
        )
        printVector(
            f,
            state_out[0].data.numpy(),
            f"test_state_out1{postfix[idx]}",
            datatype=dt,
            nFracBits=nFracBits,
        )
        printVector(
            f,
            state_out[1].data.numpy(),
            f"test_state_out2{postfix[idx]}",
            datatype=dt,
            nFracBits=nFracBits,
        )
        printVector(
            f,
            state_out[2].data.numpy(),
            f"test_state_out3{postfix[idx]}",
            datatype=dt,
            nFracBits=nFracBits,
        )
...
def generate_test_vectors(model, config, enable_testvector=True):
    if enable_testvector:
        ...
        ipt, out, state_in, state_out = inference(
            model, nn_features_tensor, 100, io_size=io_size
        )
        print_system_info_header(
            f"Max val {torch.max(nn_features_tensor)} \t Min val = {torch.min(nn_features_tensor)}"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert PyTorch Model to run in C")
    parser.add_argument(
        "-m",
        "--model_file",
        required=False,
    ...
    dump_model_to_Cfile(
        model,
        src_fname="./src/signalsifter_weights.c",
        hdr_fname="./include/signalsifter_config.h",
        w_datatype="int8_t",
        b_datatype="int8_t",
        H=hidden_size,
        io_size=io_size,
