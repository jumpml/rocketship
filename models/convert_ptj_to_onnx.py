...
def convert_to_onnx(model_file_path):
    ...
    ort_outs = ort_session.run(
        ["output", "state_out"], {"input": ipt.numpy(), "state_in": state_in.numpy()}
    )

    ort_out = ort_outs[0]
    ort_state_out = ort_outs[1]

    if (compare_tensors(ort_out, out.numpy()) == 0) & (
        compare_tensors(ort_state_out, state_out.numpy()) == 0
    ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="models/pretrained_models/jumpmlnr_pro.ptj",
    )
